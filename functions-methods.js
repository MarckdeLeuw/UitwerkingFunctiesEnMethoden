...
const email = "n.eek@novi-education.nl"
//console.log(email[5]);

function getEmailDomain(email){
    for (let i = 0; i < email.length; i++) {
        const currentLetter = email[i];
        if (currentLetter === "@"){
...
function typeOfEmail(email){
    for (let i = 0; i < email.length; i++) {
        const currentLetter = email[i];
        if (currentLetter === "@"){
            const domain = email.substring(i+1);
            //return domain;
            if (domain.includes("novi-education") ===true){
                return "Student";
            }else if (domain.includes("novi") === true){
                return "Medewerker";
            } else{
                return "Extern";
            }
